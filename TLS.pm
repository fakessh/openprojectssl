...
sub new {
	...
	print "Welcome to Net::SMTP::TLS - Debug is ON\n" if $me->{Debug};
...
sub login {
	...
	print "Supported AUTH methods: $type\n" if $me->{Debug};
	if($type =~ /CRAM\-MD5/){
	  	print "Using CRAM-MD5\n" if $me->{Debug};
		$me->auth_MD5();
	}elsif($type =~ /LOGIN/){
	  	print "Using LOGIN\n" if $me->{Debug};
		$me->auth_LOGIN();
...
sub auth_LOGIN {
	...
	print "Authentication of ", $me->{User}, " succeeded\n"
		if $me->{Debug};
}
